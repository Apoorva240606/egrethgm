...
const loginuser = asyncHandler(async (req,res) => {
    ...
    const query1 =" SELECT usn, name, address, email, number  FROM students WHERE email = '$1'  AND password = crypt('$2', password);" 
...
const getRegistrations = asyncHandler( async (req, res) => {
    ...
    const query1 = " select usn, name, batch_name, month, payment_Status from students s join payments p on s.id=p.student_id join registrations r on s.id = r.student_id where r.month between $1 and $2 "
...
const getPayments = asyncHandler( async (req, res) => {
    ...
    const { start, end} = req.body
    const query1 = " select USN,NAME,  EMAIL, NUMBER, patment_date, amount from students join payment on id=student_id where patment_date between start and end"
    try {
